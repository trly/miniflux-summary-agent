...
class TestProcessArticle:
    ...
    @pytest.mark.asyncio
    async def test_successful_processing(self, sample_article):
        ...
        with patch('ollama.Client') as mock_client_class:
            ...
            assert 'Revolutionary AI Technology Breakthrough' in call_args[1]['messages'][0]['content']
    
    @pytest.mark.asyncio
    async def test_no_tool_calls_fallback(self, sample_article):
    ...
    @pytest.mark.asyncio
    async def test_unknown_function_fallback(self, sample_article):
    ...
    @pytest.mark.asyncio
    async def test_ollama_timeout_error(self, sample_article):
...
class TestMainFunctionErrorHandling:
    """Test error handling for Miniflux and Ollama failures in main function."""
    
    @pytest.mark.asyncio
    async def test_miniflux_connection_error(self):
        """Test handling of Miniflux API connection errors."""
        with patch('miniflux.Client') as mock_miniflux, \
    ...
    @pytest.mark.asyncio
    async def test_no_recent_entries(self, sample_entries_response):
        """Test handling when no entries found within time window."""
        # Create entries with old timestamps
        old_entries = sample_entries_response.copy()
        old_timestamp = (datetime.now(timezone.utc) - timedelta(hours=12)).isoformat().replace('+00:00', 'Z')
        for entry in old_entries['entries']:
            entry['published_at'] = old_timestamp
